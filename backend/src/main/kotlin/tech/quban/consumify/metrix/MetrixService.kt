...
@Service
class MetrixService {
    private fun getSameElementsAmount(a: MutableMap<Long, Double>, b: MutableMap<Long, Double>?): Int {
        var s = 0
        for (key in a.keys) {
            if (b != null) {
    ...
    private fun SzymkiewiczSimpsonCoefficent(a: MutableMap<Long, Double>, b: MutableMap<Long, Double>): Double {
        val c = getSameElementsAmount(a, b)
        return (c.toDouble() / NumberUtils.min(a.values.size, b.values.size).toDouble())
    }

    private fun JakkarCoefficent(a: MutableMap<Long, Double>, b: MutableMap<Long, Double>): Double {
        val c = getSameElementsAmount(a, b)
        return (c.toDouble() / (a.values.size + b.values.size - c).toDouble())
    }
     ...
     */
    fun makeRecommendationByProductCos(
        userId: Long,
...
//        System.out.printf(bestSimilarity.toString())
    ...
    returns ascending map by matching product's coefficient
     */
    fun makeRecommendationByCategoryCos(
        userId: Long,
        purchases: MutableMap<Long, MutableMap<Long, Double>>,
        bestUsers: Int,
        bestProducts: Int
...
returns ascending map by matching product's coefficient
 */
    fun makeRecommendationByProductSzymkiewicz(
        userId: Long,
        purchases: MutableMap<Long, MutableMap<Long, Double>>,
        bestUsers: Int,
        bestProducts: Int
        ...
        for (entry in purchases.entries) {
            if (entry.key != userId)
                matchesWithUser.put(SzymkiewiczSimpsonCoefficent(purchases[userId]!!, entry.value), entry.key)
        }
...
//        System.out.printf(bestSimilarity.toString())
    ...
    returns ascending map by matching product's coefficient
     */
    fun makeRecommendationByCategorySzymkiewicz(
        userId: Long,
        purchases: MutableMap<Long, MutableMap<Long, Double>>,
        bestUsers: Int,
        bestProducts: Int
...
returns ascending map by matching product's coefficient
*/
    fun makeRecommendationByProductJakkar(
        userId: Long,
        purchases: MutableMap<Long, MutableMap<Long, Double>>,
        bestUsers: Int,
        bestProducts: Int
        ...
        for (entry in purchases.entries) {
            if (entry.key != userId)
                matchesWithUser.put(JakkarCoefficent(purchases[userId]!!, entry.value), entry.key)
        }
...
//        System.out.printf(bestSimilarity.toString())
    ...
    returns ascending map by matching product's coefficient
     */
    fun makeRecommendationByCategoryJakkar(
        userId: Long,
        purchases: MutableMap<Long, MutableMap<Long, Double>>,
        bestUsers: Int,
        bestProducts: Int
